...
class RemoteSupervisor:
    def __init__(
        ...
        *,
        ...
        supervisor_cwd: Path,
        supervisor_env: Optional[dict],
        default_device_id: str,
        default_webview_origin: str,
        default_cdp_origin: str,
    ) -> None:
        self.settings_path = settings_path
        self.script_path = supervisor_script
        self.supervisor_cwd = supervisor_cwd
        self.supervisor_env = supervisor_env or {}
        self.default_device_id = default_device_id
        self.default_webview_origin = default_webview_origin
        self.default_cdp_origin = default_cdp_origin
    ...
    def _default_settings(self) -> dict:
        return {
            ...
            "deviceId": self._normalize_device_id(os.environ.get("CODEX_REMOTE_DEVICE_ID", self.default_device_id)),
    ...
    def _normalize_device_id(self, value) -> str:
        ...
        return candidate or self.default_device_id
    ...
    def persist_settings(self, settings: dict) -> dict:
        ...
        payload = {**normalized, "updatedAt": int(time.time())}
    ...
    def _parse_origin(self, origin: str) -> Tuple[str, int]:
    ...
    def start(self, settings: Optional[dict] = None) -> None:
        with self.lock:
            ...
            relay_host, relay_port = self._parse_origin(effective_settings["relayOrigin"])
            rendezvous_host, rendezvous_port = self._parse_origin(effective_settings["rendezvousOrigin"])
            env = os.environ.copy()
            env.update(self.supervisor_env)
            env.update({
                "CODEX_REMOTE_MODE": effective_settings["mode"],
                "CODEX_REMOTE_LAUNCH_MODE": self._launch_mode(effective_settings),
                ...
                "CODEX_REMOTE_WEBVIEW_ORIGIN": os.environ.get("CODEX_REMOTE_WEBVIEW_ORIGIN", self.default_webview_origin),
                "CODEX_REMOTE_CDP_ORIGIN": os.environ.get("CODEX_REMOTE_CDP_ORIGIN", self.default_cdp_origin),
            })
    ...
    def apply_settings(self, payload: dict) -> dict:
        normalized = self.persist_settings({**self._read_settings(), **(payload or {})})
...
class RemoteSettingRequestHandler(SimpleHTTPRequestHandler):
    log_prefix = "webview"
    ...
    def log_message(self, format: str, *args) -> None:
        sys.stdout.write(f"[{self.log_prefix}] " + format % args + "\n")
    ...
    def do_PUT(self) -> None:
        ...
        try:
            ...
            self._respond_json(HTTPStatus.INTERNAL_SERVER_ERROR, {"error": str(error), **self.supervisor.status()})
            return
...
def run_webview_server(
    *,
    ...
    if not supervisor_cwd.is_dir():
        raise SystemExit(f"Supervisor working directory not found: {supervisor_cwd}")
    if not supervisor_script.is_file():
        raise SystemExit(f"Remote supervisor script not found: {supervisor_script}")
    ...
    supervisor = RemoteSupervisor(
        ...
        default_webview_origin=default_webview_origin,
        default_cdp_origin=default_cdp_origin,
    )
    ...
    handler_cls = type(
        "ConfiguredRemoteSettingRequestHandler",
        (RemoteSettingRequestHandler,),
        {"log_prefix": log_prefix},
    )

    def handler(*handler_args, **handler_kwargs):
        return handler_cls(*handler_args, directory=str(webview_dir), supervisor=supervisor, **handler_kwargs)

    server = ReusableThreadingHTTPServer((bind, port), handler)
    print(f"{server_description} listening on http://{bind}:{port}")
    try:
